...
public class KechenBiao extends FrameLayout {
    ...
    public KechenBiao(Context context, AttributeSet attrs) {
      ...
      //  setBackgroundResource(R.drawable.kebiao1);
  ...
  /*  private void GetAttrs_Value(Context context,AttributeSet attrs)
    {
        //与Attas取得联系
        TypedArray typedArray = context.obtainStyledAttributes(attrs,R.styleable.syllabusView);

        LineHigh = typedArray.getDimension(R.styleable.syllabusView_LineHigh, 0);
    ...
    private void SetTopBar(Context context)
    {
       ...
       // Monthbtn.setTextSize(TypedValue.COMPLEX_UNIT_DIP, TextSize);
        Monthbtn.setBootstrapSize(0.6f);
        Monthbtn.setShowOutline(true);


       // Monthbtn.setBootstrapSize(DefaultBootstrapSize.LG);
        ...
        for(int i=0;i<7;i++)
        {
          ...
          //  WeekButton[i].setBootstrapSize(DefaultBootstrapSize.LG);
          //  TopLayout.addView(WeekButton[i]);
    ...
    private void SetSectionBar(Context context)
    {
        ...
        for(int i=0;i<13;i++) {
          ...
          //  layout.setLayoutParams(new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT, 1));
           ...
           // SectionButton[i].setBootstrapBrand(bootstrapBrand[(int) (Math.random() * 7)]);
         //   SectionButton[i].setTextSize(TypedValue.COMPLEX_UNIT_DIP, 12); //***************************设置节次的字大小
            SectionButton[i].setBootstrapSize(0.55f);




          //  SectionButton[i].setHeight(150); //******************************************************** 设置节次长度

          //  layout.addView(SectionButton[i]);
       /*     Sectiontext[i] = new TextView(context);
            Sectiontext[i].setText((i + 1) + "");
            Sectiontext[i].setLayoutParams(new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT));
            Sectiontext[i].setTextSize(TypedValue.COMPLEX_UNIT_DIP, 32);
            Sectiontext[i].setBackgroundColor(Color.BLACK);
            Sectiontext[i].setTextColor(Color.WHITE);
            Sectiontext[i].setAlpha(0.4f);


            layout.addView(Sectiontext[i]);*/
    ...
    private void SetBottmBar(Context context)
    {
        ...
        for(int i=0;i<7;i++) {
           ...
           // SetColumLinearLayout_Parms(LessonLayout[i], Lesson_Weight);
           ...
           // LessonLayout[i].setOrientation(LinearLayout.VERTICAL);
